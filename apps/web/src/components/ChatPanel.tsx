...
export function ChatPanel({ targetType, targetId }: { targetType: ChatTargetType; targetId: string }) {
  ...
  return <section className="card"><h2>対象別相談チャット</h2>{loading && <p>読み込み中...</p>}{error && <p className="error">{error}</p>}<div className="chat-log">{data?.messages?.length ? data.messages.map((m, i) => <div className={`chat ${m.role}`} key={i}><strong>{m.role}</strong><p style={{ whiteSpace: "pre-wrap" }}>{m.content}</p><small>{m.createdAt}</small></div>) : <p>まだチャットはありません。</p>}</div><textarea value={message} onChange={(e: any) => setMessage(e.target.value)} placeholder="この返信案で角が立たないか、などを相談" /><button onClick={send} disabled={sending}>{sending ? "回答生成中..." : "送信"}</button>{sendError && <p className="error">{sendError}</p>}</section>;
}
