...
import { executeChat, executeCodexRun } from "./actions.js";
...
if (command === "setup") {
  ...
  console.log("agent-runner watch started (interval: 60000ms)");
  watch();
} else if (command === "chat") {
  const targetType = chatTargetTypeSchema.parse(process.argv[3] ?? process.env.TARGET_TYPE ?? "slack_message") as import("shared/types").ChatTargetType;
  const targetId = process.argv[4] ?? process.env.TARGET_ID;
  const message = process.argv.slice(5).join(" ") || process.env.CHAT_MESSAGE;
  if (!targetId || !message) throw new Error("Usage: pnpm agent:chat <slack_message|todo|reply> <targetId> <message>");
